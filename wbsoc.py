...
class _Data():
    def __init__(self,access_token:str,stonks:list,save_mode=Save.csv,directory = None,keys=None,datatype=None):
    ...
    def subscribe(self):
        ...
        if self._initialised:
            self.save_mode.open_file() # open file
            self.fyers.subscribe(symbols=self.stonks,data_type=self.data_type) # subscribe to websocket
    ...
    def unsubscribe(self):
        ...
        """
        if self._connected:
            self.fyers.unsubscribe(symbols=self.stonks, data_type=self.data_type) #unsubscribe to websocket
            self.save_mode.close_file() #close file 
...
class Depth(_Data):
    ...
    def __init__(self,access_token,stonks,directory=None,save_mode = Save.csv):
        super().__init__(access_token=access_token,
                         ...
                         directory=directory,
                         save_mode=save_mode,
                         keys=['bid_price1','bid_price2','bid_price3','bid_price4','bid_price5',
...
class Symbol(_Data):
    ...
    def __init__(self,access_token,stonks,directory=None,litemode=False,save_mode=Save.csv):
        super().__init__(access_token=access_token,
                         ...
                         directory=directory,
                         save_mode=save_mode,
                         keys=[ 'ltp', 'vol_traded_today', 'last_traded_time', 'exch_feed_time', 'bid_size', 'ask_size',
